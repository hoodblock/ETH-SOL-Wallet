...
#import <SLBProjectETHContractSDK/Payment.h>
...
const NSNotificationName SignerTokenBalanceDidChangeNotification  = @"SignerTokenBalanceDidChangeNotification";
...
static NSString *DataStoreKeyTokenBalancePrefix                   = @"BALANCE_TOKEN_";
...
static NSString *DataStoreKeyTokensPrefix                         = @"TOKENS_";
...
//    }];
    ...
    ArrayPromise *tokenBalancePromise = [_provider getTokenBalance:self.address];
    [tokenBalancePromise onCompletion:^(ArrayPromise *promise) {
        if (!promise.result || promise.error) {
            return ;
        }
        [weakSelf _setTokenBalance:promise.value];
    }];
...
- (void)_setTokenBalance:(NSArray<Erc20Token*>*)tokenBalances {
    ...
    NSDictionary *info = @{
                           SignerNotificationSignerKey: self,
                           SignerNotificationBalanceKey: tokenBalances
                           };
    dispatch_async(dispatch_get_main_queue(), ^() {
...
- (NSArray <Erc20Token *>*) tokenBalance {
    NSString *key = [DataStoreKeyTokensPrefix stringByAppendingString:self.address.checksumAddress];
    NSArray<NSDictionary*> *serialized = [_dataStore arrayForKey:key];
    NSMutableArray *tokenHistory = [NSMutableArray arrayWithCapacity:serialized.count];
    for (NSDictionary *info in serialized) {
        Erc20Token *token = [Erc20Token tokenFromDictionary:info];
        if (!token) {
            NSLog(@"Bad Token: %@", info);
            continue;
        }
        [tokenHistory addObject:token];
    }
