...
//    }
    ...
    [SLBProjectETHContract eth_sendToAssress:self.tokenAddress.text money:self.money.text tokenETH:self.segmented.selectedSegmentIndex == 0 ? nil:self.tokenAddress.text decimal:self.segmented.selectedSegmentIndex == 0 ? @"18":@"4" currentKeyStore:self.keystore.text pwd:self.pwd.text gasPrice:@"5" gasLimit:nil block:^(NSString *hashStr, BOOL suc, SLBWalletError error) {
        if (suc) {
